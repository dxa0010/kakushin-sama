...
function makeMonster() {
  ...
  const strapM  = new THREE.MeshStandardMaterial({ ...loadPBRSet("leather030", 3.5, 3.5), color: 0x3f3324, roughness: 0.76, envMapIntensity: 0.8,
  ...
  const BONES = [
    ...
    [0, 1.454, 0.406, 0, 1.442, 0.480, 0.116],   // 背骨 94°→106°（肩甲骨のヨーク（首の手前で絞る））
       ...
       ワールド座標で見たいので、必ず行列を掛けたあとに計算すること。 */
    {
      ...
      for (let i = 0; i < pos.count; i++) {
        ...
        if (o.soil) {
          ...
          const hem = 0.68 + 0.32 * Math.min(1, Math.max(0, (y - 0.40) / 0.55));
  ...
  function apronSheet(wChest, wWaist, wHem, h, bow, sway, mat, o, tWaist = 0.56, folds = 4, amp = 0.014) {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      /* 縁も v の滑らかな関数のままだと直線に見える。丈の途中で幅を揺らして、
         横の輪郭にたわみを出す。 */
      const wob = 1 + 0.030 * Math.sin(v * 13.7 + 0.9) + 0.018 * Math.sin(v * 27.1 - 1.7);
      ...
      const lane = 1 + 0.09 * Math.sin(u * 17.3 + 1.1) + 0.05 * Math.sin(u * 41.7 - 2.4);
      ...
      pos.setXYZ(i, u * w * wob, y, swell + wrink + drift + curl);
    }
    geo.computeVertexNormals();
    put(geo, mat, o);
  }
  ...
  sph(0.118, bodyM, { y: 1.062, z: -0.024, sx: 1.20, sy: 0.94, sz: 1.00, uv: 1.8, weld: true }, 24, 16);  // 尻（骨盤の後ろ下の張り）
  {
    ...
    const RAD = [0.180, 0.198, 0.204, 0.116];   // 最終区間＝ヨーク端は首の手前で絞る
    for (let i = 0; i < 4; i++) {
  ...
  for (const sd of [-1, 1]) {
    ...
    sph(0.082, bodyM, { x: sd * 0.126, y: 1.366, z: 0.264, sx: 1.06, sy: 1.44, sz: 0.46,
  ...
  tor(0.140, 0.0088, cordM, { y: HEAD.y + 0.052, z: HEAD.z, rx: Math.PI / 2, sy: 1.20, uv: 2.0, pre: headM });
  /* 結び目は**頭の横**に来る。後頭部に置くとどの角度からも見えない。 */
  tor(0.022, 0.0085, cordM, { x: -0.138, y: HEAD.y + 0.052, z: HEAD.z - 0.020, rx: 0.5, rz: 0.5, pre: headM }, 12);   // 結び目の輪
  for (const [ty, tz, tr] of [[-0.058, -0.030, 0.5], [-0.072, -0.008, 0.9]])      // 結び目から垂れる余り紐
    cap(0.0062, 0.062, cordM, { x: -0.140, y: HEAD.y + 0.052 + ty, z: HEAD.z - 0.020 + tz, rz: tr, pre: headM }, 8);
  ...
  apronSheet(0.248, 0.470, 0.430, 0.800, 0.068, -0.150, canvasM,
             { y: 0.890, z: 0.494, uv: 1.0, soil: true }, 0.62, 4, 0.022);
  ...
  for (const sd of [-1, 1]) {
    ...
    const st = bone3({ x: sd * 0.124, y: 1.246, z: 0.532 },    // 胸当ての上角（布の面の上）
                     { x: sd * 0.182, y: 1.462, z: 0.252 });   // 肩の上を外へ回して背中へ入る
    /* ※ 終点は体の**中**に入れること。表面ちょうどで止めると、丸い端の蓋が
       そのまま見えて「肩から突き出た杭」になる。 */
    cap(0.0185, st.len, strapM, { ...st, uv: 1.0 }, 10);
    /* 金具は布に沈める。厚み 1cm を布の前へ出すと、胸から突き出た杭に見える。 */
    rbox(0.022, 0.022, 0.008, 0.003, beltM, { x: sd * 0.124, y: 1.250, z: 0.540 });   // 胸当てに紐を留める金具
  }
  ...
  /* 【腰が硬い四角いベルトになっていた】革帯の箱を弧に並べていたが、決定稿の腰は
     **荒縄**で、書類の束をそこへ縛り付けている。硬い構造物を置くと、
     ...
     帯は取り払い、麻縄1本に任せる。 */
  arc(0.280, 0.280, 0.019, twineM, { y: 0.884, z: 0.192, uv: 1.0 }, -0.640, 1.280, 24);   // 腰の荒縄（束を縛る）
  ...
  for (const sd of [-1, 1]) {
    ...
    const bx = sd * 0.104, by = 0.748, bz = 0.492, brz = sd * 0.09;
    ...
    for (let i = 0; i < 10; i++) {
      const jx = (rr7() - 0.5) * 0.016, jy = (rr7() - 0.5) * 0.020;
      rbox(0.126 + rr7() * 0.022, 0.158 + rr7() * 0.028, 0.0038, 0.0012, sheafM,
           { x: bx + jx, y: by + jy, z: bz - 0.016 + i * 0.0036,
             ry: sd * 0.20 + (rr7() - 0.5) * 0.12, rz: brz + (rr7() - 0.5) * 0.11, uv: 1.0,
             noise: { amp: 0.0040, freq: 78 } });
    }
    ...
    for (let i = 0; i < 3; i++)
      /* はみ出した紙にも scrapTex（朱印の赤い丸つき）が乗っていて、束の縁に
         ...
         ここは小口と同じ生成りにする。 */
      plate(0.150 + rr7() * 0.040, 0.196 + rr7() * 0.030, sheafM,
     ...
     組み立て、最後に1つの行列で手へ持たせる。刃渡りや角度を変えても、柄と指の関係は崩れない。 */
  {
       ...
       押し出したあと頂点の z を刃幅方向の位置に応じて縮める。 */
    {
      ...
      const g2 = new THREE.ExtrudeGeometry(bs, { depth: 0.024, bevelEnabled: true, bevelThickness: 0.0035,
                                                 bevelSize: 0.0035, bevelSegments: 1, curveSegments: 4 });
      const pos2 = g2.attributes.position;
      const EDGE = -0.250, SPINE = 0.026, MID = 0.012;
    ...
    rbox(0.026, 0.366, 0.034, 0.005, cleaverM, { x: 0.014, y: -0.287, z: 0, pre: W, rust: true });   // 峰の厚み
    ...
    cyl(0.016, 0.016, 0.040, holeM, { x: -0.116, y: -0.462, z: 0.0, rx: Math.PI / 2, pre: W }, 14);  // 穴の闇
  }
  ...
  ac2.fillStyle = "#fff";
  {
    ...
    for (let k = N; k >= 0; k--) ac2.lineTo((FW * k) / N, FH - rnd(5, 42));      // 裾（いちばん裂ける）
  ...
  for (let i = 0; i < fp.count; i++) {
    ...
    const zc = Math.cos(th) * BEND - BEND + FR * pinch + lift + crin;
