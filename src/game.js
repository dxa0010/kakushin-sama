...
function makeMonster() {
     ...
     組み立て、最後に1つの行列で手へ持たせる。刃渡りや角度を変えても、柄と指の関係は崩れない。 */
  {
    ...
    for (let i = 1; i <= 6; i++) bs.lineTo(0.026, -0.104 - (0.514 * i) / 6);   // 峰（分割して蓋の三角形を細かくする）                                   // 峰（爪の付け根）
    bs.quadraticCurveTo(0.052, -0.676, 0.064, -0.754);          // 爪の背（外側・ゆるく反る）
    bs.lineTo(0.058, -0.764);                                   // 爪先（小さな面を残して角を立てる）
    bs.quadraticCurveTo(0.030, -0.716, 0.012, -0.690);          // 爪の懐（きつくえぐる）
    bs.lineTo(-0.060, -0.688);                                  // 切っ先の上の角（角として残す）
    bs.lineTo(-0.150, -0.678);
    ...
    rbox(0.026, 0.506, 0.034, 0.005, cleaverM, { x: 0.014, y: -0.357, z: 0, pre: W, rust: true });   // 峰の厚み
