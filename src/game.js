...
function makeMonster() {
  ...
  const bodyM   = new THREE.MeshStandardMaterial({ color: 0x2c2c31, roughness: 0.56, metalness: 0.05, envMapIntensity: 2.8 });  // 体（滑らかな黒）
  const bodyDk  = new THREE.MeshStandardMaterial({ color: 0x1d1d21, roughness: 0.62, metalness: 0.05, envMapIntensity: 2.2 });  // 手・靴（一段暗い）
  const jointM  = new THREE.MeshStandardMaterial({ color: 0x35353c, roughness: 0.46, metalness: 0.07, envMapIntensity: 3.4 });  // 関節の球（一段明るく艶）
  ...
  const canvasM = new THREE.MeshStandardMaterial({ normalMap: cSet.normalMap, roughnessMap: cSet.roughnessMap,
                                                   color: 0x9c8256, roughness: 0.90, metalness: 0,
  ...
  const BONE_ZS = 0.90;
  /** [ax, ay, az, bx, by, bz, r] — z を BONE_ZS で割った空間での値。
   *  胴は股関節(1.16)から肩(1.45)へ**前へ倒れた1本の軸**。背骨を波打たせない。 */
  const BONES = [
    [0, 1.080, 0.015, 0, 1.150, 0.030, 0.172],   // 骨盤
    [0, 1.150, 0.030, 0, 1.400, 0.140, 0.176],   // 腰〜背の峰（ほぼ垂直に立ち上がる）
    [0, 1.400, 0.140, 0, 1.440, 0.480, 0.180],   // 峰〜肩（ほぼ水平に前へ倒れる）
    [-0.19, 1.430, 0.480, 0.19, 1.430, 0.480, 0.148], // 肩
  ];
  ...
  function sdfBody(px, py, pz) {
    ...
    for (const [ax, ay, az, bx, by, bz, br] of BONES) {
      ...
      const apx = px - ax, apy = py - ay, apz = z - az;
  ...
  function apronSheet(wTop, wBot, h, bow, mat, o, folds = 3, amp = 0.010) {
    const geo = new THREE.PlaneGeometry(1, h, 26, 22);
    const pos = geo.attributes.position;
    for (let i = 0; i < pos.count; i++) {
      const u = pos.getX(i), y = pos.getY(i);
      const v = (y + h / 2) / h;                        // 0 = 裾, 1 = 首元
      const w = wBot + (wTop - wBot) * v;
      const q = Math.max(0, 1 - 4 * u * u);             // 中央ほど前へ張り出す
      const pz = (bow * q + amp * Math.sin(folds * Math.PI * u * 2)) * (0.34 + 0.66 * (1 - v));
      pos.setXYZ(i, u * w, y, pz);
    }
    geo.computeVertexNormals();
    put(geo, mat, o);
  }
  ...
  const HIP  = { y: 1.150, z: 0.030 };   // 股関節
  const CREST= { y: 1.432, z: 0.140 };   // 背の峰＝弧の頂点（全身でいちばん高い）
  const SHLD = { y: 1.462, z: 0.486 };   // 弧の先端＝肩
  const HEAD = { y: 1.468, z: 0.726 };   // 頭の中心。紙・革ひも・留め具はここを基準に置く
  const seg  = (a, b) => ({ y: (a.y + b.y) / 2, z: (a.z + b.z) / 2,
                            len: Math.hypot(b.y - a.y, b.z - a.z),
                            rx: Math.atan2(b.z - a.z, b.y - a.y) });
  ...
  for (const s of [-1, 1]) {
    const hx = s * 0.146, fz = s > 0 ? 0.132 : -0.104;   // 右脚を前へ、左脚を後ろへ
    sph(0.100, jointM, { x: hx, y: HIP.y - 0.020, z: 0.030 + fz * 0.20 }, 22, 14);                        // 股関節の球
    cap(0.090, 0.42, bodyM, { x: hx, y: 0.900, z: 0.036 + fz * 0.42, rx: 0.06 + fz * 0.55, uv: 1.6 }, 22); // 腿（長い）
    sph(0.086, jointM, { x: hx, y: 0.638, z: 0.062 + fz * 0.80 }, 22, 14);                                 // 膝の球
    cap(0.072, 0.40, bodyM, { x: hx, y: 0.390, z: 0.050 + fz * 0.92, rx: -0.05 + fz * 0.16, uv: 1.6 }, 22); // すね（長い）
    sph(0.060, jointM, { x: hx, y: 0.128, z: 0.052 + fz }, 18, 12);                                        // 足首の球
    rbox(0.132, 0.104, 0.268, 0.034, bodyDk, { x: hx, y: 0.058, z: 0.112 + fz, uv: 1.0 });                 // 靴（長い・厚い）
    sph(0.072, bodyDk, { x: hx, y: 0.058, z: 0.226 + fz, sx: 0.98, sy: 0.78, sz: 1.05 }, 18, 12);          // 丸いつま先
  }

  /* === 胴（股関節から前へ倒す）===
     骨盤・胴・肩の3塊。weld はこの3つにだけ掛けて1本の胴に溶かすが、
     k を 0.055 まで絞ってあるので塊の境目は線として残る。 */
  sph(0.176, bodyM, { y: HIP.y - 0.020, z: 0.026, sx: 1.16, sy: 0.94, sz: 0.94, uv: 1.8, weld: true }, 26, 16);   // 骨盤
  {
    const a = seg(HIP, CREST), b = seg(CREST, SHLD);
    cap(0.178, a.len - 0.09, bodyM, { y: a.y, z: a.z, rx: a.rx, sx: 1.12, sz: 0.90, uv: 2.0, weld: true }, 28);   // 腰→峰（立ち上がり）
    cap(0.180, b.len - 0.09, bodyM, { y: b.y, z: b.z, rx: b.rx, sx: 1.14, sz: 0.90, uv: 2.0, weld: true }, 28);   // 峰→肩（前へ倒れる）
  }
  /* 峰は全身の最高点。参照画では、いちばん高いのは頭でも肩でもなく丸めた背中で、
     頭頂はそこから 3% ほど下に並ぶ。ここを潰すと「うつむいた直立」に戻る。 */
  sph(0.178, bodyM, { y: CREST.y + 0.055, z: CREST.z + 0.020, sx: 1.10, sy: 0.90, sz: 1.06,
                      uv: 2.0, weld: true }, 28, 16);                                                             // 背の峰
  sph(0.182, bodyM, { y: SHLD.y, z: SHLD.z - 0.020, sx: 1.30, sy: 0.94, sz: 0.94,
                      uv: 2.0, weld: true }, 28, 16);                                                             // 肩の塊

  /* === 首（短い）・頭 ===
     倒れた胴の先端から 10cm 前へ出るだけ。ここを伸ばすと決定稿ではなくなる。 */
  cap(0.070, 0.06, bodyM, { y: 1.466, z: 0.610, rx: -1.55, uv: 1.2 }, 18);                       // 首（短い・ほぼ水平）
  ...
  sph(0.156, bodyM, { y: HEAD.y, z: HEAD.z, sx: 0.96, sy: 1.06, sz: 1.00, pre: headM }, 26, 18);
  // 紙を留めている革ひも。決定稿では頭の上寄りに1本だけ横へ回している
  tor(0.172, 0.010, strapM, { y: HEAD.y + 0.072, z: HEAD.z, rx: Math.PI / 2, sy: 1.02, uv: 2.0, pre: headM });
  ...
  const arm = (s, fa, grip) => {
    const EL = { y: 1.062, z: 0.352 };                      // 肘の位置
    const L  = 0.212;                                       // 前腕の半長（キャップ込み）
    const fy = EL.y - L * Math.cos(fa), fz = EL.z + L * Math.sin(fa);          // 前腕の中心
    const wy = EL.y - 2 * L * Math.cos(fa), wz = EL.z + 2 * L * Math.sin(fa);  // 手首
    sph(0.098, jointM, { x: s * 0.252, y: SHLD.y, z: 0.480 }, 22, 14);                                  // 肩の球
    cap(0.074, 0.32, bodyM, { x: s * 0.266, y: 1.258, z: 0.418, rx: 0.30, uv: 1.6 }, 22);               // 上腕（下がりつつ後ろへ）
    sph(0.070, jointM, { x: s * 0.276, y: EL.y, z: EL.z }, 20, 14);                                      // 肘の球
    cap(0.062, 0.30, bodyM, { x: s * 0.286, y: fy, z: fz, rx: -fa, uv: 1.6 }, 22);                       // 前腕（下がりつつ前へ）
    const hx = s * 0.292, hy = wy - 0.052, hz = wz + 0.030;
    sph(0.052, jointM, { x: hx, y: wy, z: wz }, 16, 12);                                                 // 手首の球
    rbox(0.062, 0.044, 0.086, 0.015, bodyDk, { x: hx, y: hy, z: hz, rx: -0.10 - fa * 0.30, uv: 1.0 });   // 手の甲
    /* 決定稿の指は細長い。垂れている手は指を伸ばして下へ、握っている手は
       柄に巻き付ける。第1・第2関節の2節だけで、爪も指先の球も置かない。 */
    for (let i = 0; i < 4; i++) {
      const fx = hx + (i - 1.5) * 0.021 * s;
      const t = 1 - Math.abs(i - 1.1) * 0.10;
      if (grip) {
        cap(0.0130 * t, 0.034 * t, bodyDk, { x: fx, y: hy - 0.028, z: hz + 0.042, rx: 1.10, uv: 0.8 }, 8);
        cap(0.0120 * t, 0.028 * t, bodyDk, { x: fx, y: hy - 0.058, z: hz + 0.036, rx: 0.20, uv: 0.8 }, 8);
      } else {
        cap(0.0132 * t, 0.050 * t, bodyDk, { x: fx, y: hy - 0.064, z: hz + 0.010, rx: 0.10, uv: 0.8 }, 8);
        cap(0.0116 * t, 0.042 * t, bodyDk, { x: fx, y: hy - 0.148, z: hz + 0.018, rx: 0.16, uv: 0.8 }, 8);
      }
    }
    cap(0.015, 0.038, bodyDk, { x: hx - s * 0.042, y: hy - 0.024, z: hz + 0.026, rz: s * 0.85, rx: -0.35, uv: 0.8 }, 8);  // 親指
    return { x: hx, y: hy, z: hz };
  };
  /* 大鉈は**画面左手**に持たせる。決定稿がそうなっている。四面図の front は
     怪人を π 回して撮るので、怪人のローカル -x が画面の左に出る。
     握る側は肘を深く曲げて（fa=0.86）刃を体の前へ送り、
     垂れる側は浅く（fa=0.34）指を伸ばして膝の高さまで落とす。 */
  /* 握る側の屈曲は 0.86 → 0.55 に浅くした。0.86 だと拳が z=0.70 まで前へ出て、
     そこから刃を下ろすと切っ先が体の 1m 先に着き、奥行きが参照の 1.6 倍になる。
     浅く曲げて拳を体寄り・低めに置くと、同じ刃渡りで切っ先が床に届く。 */
  const GRIP = arm(-1, 0.55, true);
  arm(1, 0.34, false);
  ...
  apronSheet(0.226, 0.402, 0.88, 0.052, canvasM, { y: 0.995, z: 0.512, rx: 0.03, uv: 1.0 });
  for (const sd of [-1, 1])                    // 首から掛ける吊り紐
    cap(0.013, 0.26, canvasM, { x: sd * 0.100, y: 1.404, z: 0.508, rx: -0.42, rz: sd * 0.20, uv: 1.0 }, 8);
  cap(0.012, 0.36, twineM, { y: 1.030, z: 0.566, rz: Math.PI / 2, uv: 1.0 }, 8);   // 腰で締める紐

  /* === 腰の書類の束2つ ===
     決定稿で唯一の持ち物。前掛けの上に、紐で十字に縛って留めてある。
     白い点を**顔とこの2つだけ**に絞ることで、暗闇で追うべき箇所が決まる。 */
  for (const sd of [-1, 1]) {
    const bx = sd * 0.128, by = 0.968, bz = 0.596;
    rbox(0.162, 0.118, 0.050, 0.005, paperM, { x: bx, y: by, z: bz, rz: sd * 0.05, uv: 1.0 });
    for (let i = 0; i < 4; i++)                // 紙の層。縁をわずかにずらして「束」に見せる
      plate(0.154 - i * 0.005, 0.111, paperM,
            { x: bx + (i - 1.5) * 0.003, y: by + (i - 1.5) * 0.004, z: bz + 0.026 + i * 0.0012, rz: sd * 0.05 });
    cap(0.0055, 0.150, twineM, { x: bx, y: by, z: bz + 0.030, rz: Math.PI / 2, uv: 1.0 }, 6);   // 横に回した紐
    cap(0.0055, 0.106, twineM, { x: bx, y: by, z: bz + 0.030, uv: 1.0 }, 6);                    // 縦に回した紐
  }
  ...
  /* 【rx の符号】刃は柄の原点から局所 -y へ伸びる。rotX(+a) は (0,-1,0) を
     (0, -cos a, -sin a) へ送るので、**正の rx では刃が後ろへ倒れる**。
     ここをずっと正にしていたため、切っ先が脚の後ろへ回り込み、
     引きずるどころか背後に隠れて、横から見ると細い棒にしか見えなかった。
     負にすると前へ倒れる。刃渡り 0.71m を rx=-0.30 で倒すと、
     拳(y≈0.66)から 0.68m 落ちて床に着き、前へ 0.21m 出る。 */
  const mac = new THREE.Matrix4().compose(
    new THREE.Vector3(GRIP.x, GRIP.y + 0.010, GRIP.z + 0.018),
    new THREE.Quaternion().setFromEuler(new THREE.Euler(-0.26, 0.16, 0.22)),
    new THREE.Vector3(1, 1, 1));
  cyl(0.024, 0.027, 0.22, handleM, { y: 0.048, pre: mac }, 14);        // 柄（拳を貫通させる）
  cyl(0.033, 0.030, 0.026, handleM, { y: 0.168, pre: mac }, 14);       // 柄頭
  ...
  const FR = 0.174, FPW = 0.285, FPH = 0.360;    // 巻き付ける半径・紙の幅・高さ（頭の表面 0.156 より外側）
  ...
  face.position.set(0, HEAD.y - 0.093, HEAD.z);   // 上端が頭の上寄り・下端が顎の下に来る高さ
  ...
  for (const [tx, ty] of [[-0.078, 0.146], [0.078, 0.146]]) {
    const th = tx / FR, p = tack.clone();
    p.position.set(Math.sin(th) * FR, HEAD.y - 0.093 + ty, HEAD.z + Math.cos(th) * FR);
