...
function makeMonster() {
  ...
  const BONES = [
    ...
    [0, 1.030, -0.064, 0, 1.168, 0.022, 0.152],  // 尻 → 股関節（脚の付け根と繋ぐ）
    [0, 1.030, -0.064, 0, 1.150, 0.010, 0.140],  // 尻の後ろ下の膨らみ
    [0, 1.168, 0.022, 0, 1.339, 0.077, 0.172],   // 背骨 0°→32°（腰）
    [0, 1.339, 0.077, 0, 1.458, 0.224, 0.184],   // 背骨 32°→64°（中背）
    [0, 1.458, 0.224, 0, 1.490, 0.407, 0.182],   // 背骨 64°→94°（背の峰）
    [0, 1.490, 0.407, 0, 1.442, 0.571, 0.152],   // 背骨 94°→122°（肩甲骨のヨーク）
  ...
  /* 【猫背は「くの字」ではなく「Cの字」】前の版は腰から上をほぼ垂直に立ち上げ、
     ...
     なく、**弧が頭を運ぶ**。 */
  const HIP  = { y: 1.168, z: 0.020 };            // 股関節＝弧の始点
  ...
  const YOKE  = spine(122);   // 弧の終端＝肩甲骨のヨーク＝首の付け根
  /* 頭は肩より **下**にある。参照では頭の中心が肩より一段低く、
     覗き込むというより「頭が垂れている」。前の版は肩と同じ高さだった。 */
  const HEAD = { y: 1.399, z: 0.660 };   // 頭の中心（肩より 4cm 下）
  ...
  for (const s of [-1, 1]) {
    ...
    const kz = fwd ? 0.290 : -0.105, az = fwd ? 0.370 : -0.158;
    ...
    const kn = { x: s * 0.222, y: 0.594,  z: HIP.z + kz };
    const an = { x: s * 0.240, y: 0.112,  z: HIP.z + az };
    const th = bone3(hp, kn), sk = bone3(kn, an);
    sph(0.084, jointM, hp, 22, 14);                                                       // 股関節の球
    cap(0.068, th.len - 0.156, bodyM, { ...th, uv: 1.6 }, 20);                            // 腿（長い・細い）
    sph(0.058, jointM, kn, 20, 14);                                                       // 膝の球
    cap(0.050, sk.len - 0.120, bodyM, { ...sk, uv: 1.6 }, 20);                            // すね
    sph(0.046, jointM, an, 16, 12);                                                       // 足首の球
  ...
  sph(0.145, bodyM, { y: 1.055, z: -0.045, sx: 1.22, sy: 0.92, sz: 1.00, uv: 1.8, weld: true }, 26, 16);  // 尻（骨盤の後ろ下）
  {
    /* 弧を4区間に切って並べる。隣どうしの角度差が 32/32/30/28 度と揃っているので、
       繋ぎ目が「折れ目」ではなく「曲率」として読める。半径は中背をいちばん太くして
       ヨークで絞る――決定稿の胴は背の峰あたりがもっとも分厚い。 */
    const SEG = [0, 32, 64, 94, 122];
    const RAD = [0.172, 0.184, 0.182, 0.152];
    for (let i = 0; i < 4; i++) {
     ...
     ここを細く長い管にすると、猫背の怪人ではなく鶴になる。 */
  {
   ...
   // ヨーク（肩甲骨まわりの塊）
    ...
    cap(0.062, nk.len + 0.03, bodyM, { ...nk, uv: 1.4 }, 20);                         // 首（短く・細い）
  }
  ...
  const arm = (s, grip) => {
    ...
    const SH = { x: s * 0.332, y: 1.455, z: 0.330 };   // 肩（三角筋の中心）
    const EL = { x: s * 0.356, y: 0.960, z: 0.250 };   // 肘（後ろへ張り出す）
    const WR = { x: s * 0.340, y: 0.505, z: 0.330 };   // 手首（前へ戻る）
    const ua = bone3(SH, EL), fo = bone3(EL, WR);
    sph(0.082, jointM, SH, 22, 14);                                          // 肩の球（三角筋）
    cap(0.062, ua.len - 0.144, bodyM, { ...ua, uv: 1.6 }, 20);               // 上腕
    sph(0.062, jointM, EL, 20, 14);                                          // 肘の球
    cap(0.054, fo.len - 0.128, bodyM, { ...fo, uv: 1.6 }, 20);               // 前腕
    sph(0.046, jointM, WR, 16, 12);                                          // 手首の球
  ...
  apronSheet(0.300, 0.332, 0.492, 0.975, 0.125, canvasM,
             { y: 0.768, z: 0.392, rx: 0.10, uv: 1.0 }, 0.60, 4, 0.014);
  ...
  for (const sd of [-1, 1]) {
    ...
    const st = bone3({ x: sd * 0.150, y: 1.252, z: 0.516 },    // 胸当ての上角（布の面の上）
                     { x: sd * 0.105, y: 1.474, z: 0.510 });   // ヨークの上（首の付け根）
    cap(0.014, st.len, strapM, { ...st, uv: 1.0 }, 8);
    rbox(0.030, 0.030, 0.010, 0.004, strapM, { x: sd * 0.150, y: 1.248, z: 0.524 });   // 胸当てに紐を留める金具
  }
