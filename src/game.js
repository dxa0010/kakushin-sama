...
function makeMonster() {
  ...
  const cleaverM= new THREE.MeshStandardMaterial({ color: 0x8a7263, roughness: 0.52, metalness: 0.45, envMapIntensity: 3.0 });   // 錆びた刃
  ...
  const BONES = [
    ...
    [0, 1.410, 0.407, 0, 1.384, 0.535, 0.108],   // 背骨 94°→116°（肩甲骨のヨーク／首の手前で絞る）
  ...
  const YOKE  = spine(116);   // 弧の終端＝肩甲骨のヨーク＝首の付け根
  ...
  const HEAD = { y: 1.344, z: 0.800 };   // 頭の中心（肩とほぼ同じ高さ・前）
  ...
  for (const s of [-1, 1]) {
    ...
    const kz = fwd ? 0.290 : 0.130, az = fwd ? 0.480 : -0.310;
    ...
    const an = { x: s * 0.252, y: 0.112,  z: HIP.z + az };
  ...
  tor(0.146, 0.012, cordM, { y: HEAD.y + 0.046, z: HEAD.z, rx: Math.PI / 2, sy: 1.30, uv: 2.0, pre: headM });
  ...
  /* 【刃を布から逃がす方向を間違えていた】前掛けと刃の交差を避けようと、
     ...
     刃は拳から素直に下がる。 */
  const BOFF = { x: -0.062, y: 0.352, z: 0.028 };   // 拳 → 刃の付け根（外側・少し上）
  const arm = (s, grip) => {
    ...
    rbox(0.058, 0.104, 0.072, 0.016, bodyDk, { x: P.x, y: P.y, z: P.z, rx: 0.08, uv: 1.0 });   // 手の甲
  ...
  apronSheet(0.244, 0.300, 0.430, 0.800, 0.112, -0.105, canvasM,
             { y: 0.960, z: 0.634, uv: 1.0 }, 0.62, 5, 0.018);
  ...
  const bDir = new THREE.Vector3(0.40, -0.78, 0.24).normalize();   // 付け根 → 切っ先
  ...
  put(new THREE.ExtrudeGeometry(bs, { depth: 0.020, bevelEnabled: true, bevelThickness: 0.003, bevelSize: 0.003, bevelSegments: 1, curveSegments: 4 }),
      cleaverM, { z: -0.010, pre: mac });
  ...
  ac2.fillStyle = "#000";
  for (const ex of [116, 316]) ac2.fillRect(ex + 6, 241, 68, 52);
  ...
  /* 丈は 0.372 → 0.298。背骨を弧にして頭を 3cm 下げたことで、同じ丈のままだと
     ...
     参照では紙の下端は顎のすぐ下で終わる。 */
  const FR = 0.170, FPW = 0.302, FPH = 0.298;    // 巻き付ける半径・紙の幅・高さ（頭の表面 0.153 より外側）
