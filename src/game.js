...
function makeMonster() {
  ...
  const WGRIP = { x: -0.026, y: 0.150, z: 0.016 };
  ...
  const WDIR  = new THREE.Vector3(0.52, -0.470, 0.700).normalize();
  const WROLL = 0.15;
  const arm = (s, grip) => {
     ...
     組み立て、最後に1つの行列で手へ持たせる。刃渡りや角度を変えても、柄と指の関係は崩れない。 */
  {
    ...
    cyl(0.033, 0.030, 0.340, handleM, { y: -0.030, pre: W }, 16);     // 柄（拳を貫通して下へ伸びる）
    sph(0.038, handleM, { y: 0.150, pre: W }, 14, 10);                 // 柄頭
    for (const ty of [-0.032, 0.052]) {                                // 革巻き（握る場所を示す）
      sph(0.037, strapM, { y: ty, sy: 0.34, pre: W }, 14, 8);
    }
    rbox(0.072, 0.024, 0.030, 0.006, bladeM, { y: -0.172, pre: W });   // 口金

    /* --- 刃 --- 原点から下へ伸ばす。刃渡りは握りから 0.62m。
       参照の角度（水平から 36 度）で下ろすと落差 0.365m で、拳(y≒0.38)から床に触れる。 */
    const bs = new THREE.Shape();
    bs.moveTo(0.022, -0.186);                                   // 峰（柄側）
    bs.lineTo(0.022, -0.560);                                   // 峰（先端側）
    bs.quadraticCurveTo(0.022, -0.624, -0.046, -0.628);         // 先端の上の角
    bs.lineTo(-0.142, -0.628);
    bs.quadraticCurveTo(-0.201, -0.628, -0.203, -0.574);        // 先端の下の角（大きく丸める）
    bs.lineTo(-0.198, -0.186);                                  // 刃（柄側）
    bs.closePath();
    ...
    rbox(0.024, 0.400, 0.032, 0.006, cleaverM, { x: 0.010, y: -0.386, z: 0, pre: W });   // 峰の厚み
    rbox(0.012, 0.392, 0.018, 0.003, edgeM,    { x: -0.194, y: -0.388, z: 0, pre: W });  // 研いだ刃先
    tor(0.019, 0.005, cleaverM, { x: -0.056, y: -0.556, z: 0.0, pre: W }, 12);           // 先端の穴のふち
  }
