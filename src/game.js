...
function makeMonster() {
  ...
  const beltM   = new THREE.MeshStandardMaterial({
    ...
    color: 0x51402c, roughness: 0.66, metalness: 0.0, envMapIntensity: 1.3 });   // 腰帯・金具
  ...
  const jointM  = new THREE.MeshStandardMaterial({ ...skinSurf(6, 0.26), color: 0x333338, roughness: 0.84, metalness: 0.0, envMapIntensity: 1.25 });  // 関節の球
  ...
  rcv.width = 256; rcv.height = 512;
  {
    ...
    // 広い錆の斑。濃さと大きさを散らして、繰り返しに見えないようにする
    for (let i = 0; i < 150; i++) {
      const x = rnd() * 256, y = rnd() * 512, r = 14 + rnd() * 62;
      const g = rc.createRadialGradient(x, y, 0, x, y, r);
      const a = 0.16 + rnd() * 0.52;
      g.addColorStop(0, `rgba(${100 + rnd() * 40 | 0},${48 + rnd() * 26 | 0},26,${a})`);
  ...
  const cleaverM= new THREE.MeshStandardMaterial({
    ...
    color: 0x7d6e60, roughness: 0.76, metalness: 0.26, envMapIntensity: 0.26 });   // 錆びた刃
  ...
  ahv.width = 512; ahv.height = 256;
  {
    ...
    const ae = [];
    for (let k = 0; k <= 32; k++) ae.push(k === 0 || k === 32 ? 236 : 224 + rr7() * 22);
    for (let k = 32; k >= 0; k--) ah.lineTo(k * 16, ae[k]);
    ah.closePath(); ah.fill();
    for (let k = 0; k < 34; k++) {               // 裾から垂れるほつれ糸（短く）
      const tx = rr7() * 512, base = ae[Math.min(32, Math.round(tx / 16))];
      ah.fillRect(tx, base - 4, 1.2 + rr7() * 2.4, 2 + rr7() * 13);
    }
  ...
  apronHemTex.wrapS = THREE.RepeatWrapping; apronHemTex.repeat.set(2, 1);
  ...
  const BONES = [
    ...
    [0, 1.048, -0.030, 0, 1.132, 0.020, 0.118],  // 尻（骨盤の後ろ下の張り）
    ...
    [0, 1.132, 0.022, 0, 1.303, 0.077, 0.180],   // 背骨 0°→32°（腰）
    [0, 1.303, 0.077, 0, 1.422, 0.224, 0.198],   // 背骨 32°→64°（中背）
    [0, 1.422, 0.224, 0, 1.454, 0.406, 0.204],   // 背骨 64°→94°（背の峰）
    [0, 1.454, 0.406, 0, 1.442, 0.480, 0.132],   // 背骨 94°→106°（肩甲骨のヨーク（首の手前で絞る））
       ...
       ワールド座標で見たいので、必ず行列を掛けたあとに計算すること。 */
    {
      ...
      for (let i = 0; i < pos.count; i++) {
        ...
        if (o.rust) {
          ...
          const mottle = 0.50 + 0.14 * (0.5 + 0.5 * (n1 * 0.72 + n2 * 0.28));
  ...
  function apronSheet(wChest, wWaist, wHem, h, bow, sway, mat, o, tWaist = 0.56, folds = 4, amp = 0.014) {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      const wrink = amp * (0.25 + 1.35 * (1 - v)) * (1 - 0.75 * cinch)
                  * (0.62 * Math.sin(folds * Math.PI * u * 2 + v * 0.9)
                   + 0.38 * Math.sin(folds * 1.73 * Math.PI * u * 2 + 2.1 - v * 0.6));
  ...
  const HEAD = { y: 1.470, z: 0.664 };   // 頭の中心（ヨークの前・やや上）
  ...
  const handPart = (O, dir, s, grip) => {
    ...
    sph(0.052, bodyDk, { sx: 0.86, sy: 1.30, sz: 1.05, rx: 0.08, uv: 1.0, pre: H }, 18, 12);
    ...
    if (grip) {
      ...
      const u = new THREE.Vector3(g.dot(hX), g.dot(hY), g.dot(hZ)).normalize();
      const ref = Math.abs(u.y) < 0.9 ? new THREE.Vector3(0, 1, 0) : new THREE.Vector3(1, 0, 0);
      const e1 = new THREE.Vector3().crossVectors(ref, u).normalize();
      const e2 = new THREE.Vector3().crossVectors(u, e1);
      /* 【マジックハンドに見えた】指の輪を柄（半径 0.033）の 1.8 倍まで広げたら、
         指だけが外へ張り出して機械の爪になった（agy）。柄の表面に触れる値に詰め、
         付け根は手のひらの塊で隠す。 */
      const R = 0.040;
      for (let i = 0; i < 4; i++) {
        ...
        const rad = e1.clone().multiplyScalar(Math.cos(a)).addScaledVector(e2, Math.sin(a));
        const c = rad.clone().multiplyScalar(R).addScaledVector(u, along);
        // 指は柄を横切る向き。軸に直交する面内で、接線方向に寝かせる
        const d = bone3({ x: c.x, y: c.y, z: c.z },
                        { x: c.x - rad.x * 0.052, y: c.y - rad.y * 0.052, z: c.z - rad.z * 0.052 });
        cap(0.0135 * t, d.len, bodyDk, { ...d, uv: 0.8, pre: H }, 8);
      }
      // 親指は反対側から柄を押さえる
      const a = s * 1.35;
      const c = e1.clone().multiplyScalar(Math.cos(a)).addScaledVector(e2, Math.sin(a)).multiplyScalar(R);
      cap(0.016, 0.052, bodyDk, { x: c.x, y: c.y, z: c.z, rz: s * 0.9, rx: -0.3, uv: 0.8, pre: H }, 8);
    } else {
      /* 垂れている手。
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const B = { x: fx, y: 0.040 + 0.054 * t, z: -0.016 };
        const C = { x: fx, y: 0.040 + 0.096 * t, z: -0.048 };
        const d1 = bone3(A, B), d2 = bone3(B, C);
        cap(0.0132 * t, d1.len, bodyDk, { ...d1, uv: 0.8, pre: H }, 8);
        cap(0.0110 * t, d2.len, bodyDk, { ...d2, uv: 0.8, pre: H }, 8);
      }
      // 親指は手のひらの側面から生やす。指と同じく2点を結ぶ
      const T0 = { x: -s * 0.030, y: 0.012, z: -0.010 };
      const T1 = { x: -s * 0.050, y: 0.062, z: -0.038 };
      const dt = bone3(T0, T1);
      cap(0.0148, dt.len, bodyDk, { ...dt, uv: 0.8, pre: H }, 8);
    }
  };
  ...
  const bootPart = (an, s) => {
    ...
    rbox(0.120, 0.098, 0.156, 0.030, bodyDk, { x: s * 0.016, y: 0.056, z: 0.104, uv: 1.0, pre: F });  // 爪先
  ...
  sph(0.118, bodyM, { y: 1.062, z: -0.024, sx: 1.20, sy: 0.94, sz: 1.00, uv: 1.8, weld: true }, 24, 16);  // 尻（骨盤の後ろ下の張り）
  {
    ...
    const RAD = [0.180, 0.198, 0.204, 0.132];   // 最終区間＝ヨーク端は首の手前で絞る
    for (let i = 0; i < 4; i++) {
  ...
  sph(0.150, bodyM, { y: CREST.y - 0.012, z: CREST.z - 0.020, sx: 1.34, sy: 0.72, sz: 1.22,
  ...
  for (const sd of [-1, 1]) {
    ...
    sph(0.116, bodyM, { x: sd * 0.130, y: 1.372, z: 0.262, sx: 0.94, sy: 1.30, sz: 0.22,
  ...
  tor(0.146, 0.0062, cordM, { y: HEAD.y + 0.052, z: HEAD.z, rx: Math.PI / 2, sy: 1.00, uv: 2.0, pre: headM });
  ...
  /* ================= 大鉈の持ち方 =================
     ...
     WGRIP  握りの中心を、拳（前腕の延長で決まる P）からどれだけずらすか
  ...
  const WGRIP = { x: -0.026, y: 0.015, z: 0.016 };
  ...
  const WDIR  = new THREE.Vector3(0.88, -0.22, 0.42).normalize();
  const WROLL = 0.05;
  const arm = (s, grip) => {
    ...
    const SH = { x: s * 0.340, y: 1.318, z: 0.352 };   // 肩（三角筋の中心）
    ...
    handPart(P, fd, s, grip ? WDIR.clone().negate() : null);
    return P;
  };
  ...
  apronSheet(0.216, 0.410, 0.368, 0.876, 0.068, -0.235, canvasM,
             { y: 0.852, z: 0.494, uv: 1.0 }, 0.62, 4, 0.032);
  ...
  for (const sd of [-1, 1]) {
    ...
    const st = bone3({ x: sd * 0.132, y: 1.232, z: 0.542 },    // 胸当ての上角（布の面の上）
                     { x: sd * 0.112, y: 1.492, z: 0.296 });   // 肩の上を越えて背中側へ
    cap(0.0135, st.len, strapM, { ...st, uv: 1.0 }, 10);
    rbox(0.026, 0.026, 0.010, 0.004, beltM, { x: sd * 0.132, y: 1.238, z: 0.560 });   // 胸当てに紐を留める金具
  }
  ...
  {
    /* 前掛けの丈を変えると、同じ高さでも布の前後位置（裾へ向かう後退の途中）が動く。
       腰の高さの布面は z≒0.487 に来るので、弧の中心をそこから半径ぶん引いた位置に置く。 */
    /* ※ 布面（z≒0.487）を**またぐ**位置に置くこと。
       ・後ろへ入れると帯が布に埋まり、布の裏だけが黒い帯として残る
       ・前へ出しすぎると 2cm の棚になり、上からの光で前面が完全に影になって
         「黒い箱を並べた鍵盤」に見える（実際そうなった）
       布から 8mm だけ顔を出す厚みにして、帯として読ませる。 */
    const BR = 0.573, BY = 1.048, BZ = -0.085;     // 弧の半径・中心（前掛けの曲率に合わせてある）
    /* 帯は布の**内側**で終わらせる。布の縁は内へ巻き込んであるので、
       縁いっぱいまで伸ばすと帯だけが布から外れて横へ突き出して見える。 */
    const T0 = -0.250, T1 = 0.250, N = 7;
    for (let i = 0; i < N; i++) {
      const t = T0 + (T1 - T0) * (i + 0.5) / N;
      /* CylinderGeometry と同じ向き（θ=0 が +z）に合わせる。接線は (cos t, 0, -sin t)
         なので、箱を y 軸まわりに t だけ回すと +x が接線に乗る。 */
      rbox(BR * (T1 - T0) / N + 0.020, 0.052, 0.014, 0.005, beltM,
           { x: BR * Math.sin(t), y: BY, z: BZ + BR * Math.cos(t), ry: t, uv: 1.0 });
    }
  }
  ...
  for (const sd of [-1, 1]) {
    ...
    const bx = sd * 0.104, by = 0.786, bz = 0.492, brz = sd * 0.09;
    ...
    /* 角丸を 0.006 → 0.018 に。直方体のままだと「箱」にしか見えない。
       紙の束は角が潰れて丸い。 */
    rbox(0.146, 0.184, 0.032, 0.016, sheafM,
         { x: bx, y: by, z: bz, ry: sd * 0.20, rz: brz, uv: 1.0,
    ...
    for (let i = 0; i < 3; i++)
      plate(0.150 + rr7() * 0.040, 0.196 + rr7() * 0.030, paperM,
            { x: bx + (rr7() - 0.5) * 0.030, y: by + (rr7() - 0.5) * 0.040,
              z: bz - 0.030 - i * 0.008,
              ry: sd * 0.20 + (rr7() - 0.5) * 0.18, rz: brz + (rr7() - 0.5) * 0.16 });
    ...
    plate(0.170, 0.222, sheafM,
          { x: bx + sd * 0.008, y: by + 0.002, z: bz + 0.030, ry: sd * 0.20, rz: brz });
    ...
    cap(0.0072, 0.196, twineM, { x: bx, y: by, z: bz + 0.020, rz: Math.PI / 2 + brz, ry: sd * 0.20, uv: 1.0 }, 8);   // 横に回した紐
    cap(0.0072, 0.238, twineM, { x: bx, y: by, z: bz + 0.020, rz: brz, ry: sd * 0.20, uv: 1.0 }, 8);            // 縦に回した紐
  }
     ...
     組み立て、最後に1つの行列で手へ持たせる。刃渡りや角度を変えても、柄と指の関係は崩れない。 */
  {
    ...
    const wO = new THREE.Vector3(GRIP.x + WGRIP.x, GRIP.y + WGRIP.y, GRIP.z + WGRIP.z);
    ...
    cyl(0.031, 0.028, 0.215, handleM, { y: -0.005, pre: W }, 16);     // 柄（拳の中心を貫通する）
    ...
    bs.lineTo(0.026, -0.470);                                   // 峰（先端側）
    bs.quadraticCurveTo(0.026, -0.536, -0.044, -0.540);         // 切っ先の上の角
    bs.lineTo(-0.150, -0.538);
    bs.quadraticCurveTo(-0.230, -0.534, -0.240, -0.462);        // 切っ先の下の角
    bs.quadraticCurveTo(-0.250, -0.300, -0.212, -0.158);        // 刃（腹をふくらませる）
       ...
       押し出したあと頂点の z を刃幅方向の位置に応じて縮める。 */
    {
      ...
      const EDGE = -0.250, SPINE = 0.026, MID = 0.010;
      ...
      put(g2, cleaverM, { z: -0.010, pre: W, rust: true, uv: 3.0 });
    }
    rbox(0.024, 0.366, 0.032, 0.006, cleaverM, { x: 0.014, y: -0.287, z: 0, pre: W, rust: true });   // 峰の厚み
    /* 刃先の明るい線は**腹の曲線の内側**に置く。直線の棒なので、曲線から
       はみ出さない範囲（腹がいちばん張り出す y=-0.30 のあたり）だけに限る。 */
    rbox(0.012, 0.220, 0.018, 0.003, edgeM,    { x: -0.220, y: -0.330, z: 0, pre: W });  // 研いだ刃先
    tor(0.019, 0.005, cleaverM, { x: -0.116, y: -0.462, z: 0.0, pre: W, rust: true }, 12);  // 先端の穴のふち
  }
  ...
  const FR = 0.164, FPW = 0.272, FPH = 0.282;    // 巻き付ける半径・紙の幅・高さ
  ...
  for (let i = 0; i < fp.count; i++) {
    ...
    /* 0.28 まで巻いたら今度は「顔の前にはめた硬い筒状のカバー」になった（agy）。
       ...
       平らな札にも筒にもならない幅。 */
    const BEND = 0.34;                                     // 紙の曲がりの半径（大きいほど平ら）
