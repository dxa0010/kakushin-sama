...
function makeMonster() {
  ...
  const WDIR  = new THREE.Vector3(0.88, -0.16, 0.42).normalize();
  const WROLL = 0.05;
  const arm = (s, grip) => {
     ...
     組み立て、最後に1つの行列で手へ持たせる。刃渡りや角度を変えても、柄と指の関係は崩れない。 */
  {
    ...
    /* ※ 最初は付け根を -0.548 まで下げ、爪先を峰の 5.4cm 上・懐を 11cm 手前に取ったが、
       細長い三角形が立っただけで「刃の先に旗が付いた」ようになり、懐が深すぎて
       切っ先が欠けたようにも見えた。爪付き鉈の爪は**短く太い**。
       付け根を切っ先の近く（-0.618）へ寄せ、爪先の持ち上がりを 3.6cm に抑え、
       懐は切っ先の前面とほぼ同じ高さで**きつく**えぐる。 */
    /* 【面に斜めの稜線が1本走っていた】ExtrudeGeometry の前後の蓋は**耳切り**で
       三角形に割られる。峰が -0.104 から -0.618 まで**1本の長い辺**だったので、
       蓋が数個の細長い三角形になり、そこへ下の楔（頂点の z を x に応じて縮める処理）を
       掛けたせいで面が平面でなくなり、長い対角線が稜線として画に出ていた。
       爪（凹んだ頂点）を足して耳切りの結果が変わり、はっきり見えるようになった。
       ※ 影を切っても消えないので、シャドウアクネではない（--noshadow で確認済み）。
       直し方は「楔をやめる」ではなく**輪郭の点を増やす**こと。三角形が小さくなれば
       computeVertexNormals の平均が真の面法線に近づき、稜線は消える。 */
    for (let i = 1; i <= 6; i++) bs.lineTo(0.026, -0.104 - (0.499 * i) / 6);   // 峰（分割して蓋の三角形を細かくする）                                   // 峰（爪の付け根）
    /* 【真っ直ぐ突き出すと「槍先」で、鉤にならない】峰の延長線上へ直線的に尖らせると、
       柄・峰・爪が一直線に並ぶので、爪ではなく**柄がそのまま刃を貫いて突き出した棒**に
       見える（依頼主「柄から爪が出てます」、agy「フックとして物を引っ掛けられず、
       単なるトゲか槍の先端に見える」）。
       爪付き鉈の爪は**先端が曲がっている**。峰の先から前へ伸びたあと、切っ先を
       刃の側（下）へ折り返すことで、爪と切っ先の前面のあいだに**引っ掛けるための懐**ができる。
       この折り返しが有るか無いかが、鉤と槍先の分かれ目。 */
    /* ※ 折り返しを「峰より 1cm 下」までにしたら、まだ「峰の延長」「太いトゲ」と読まれた。
       折り返しは**峰より下へ抜ける**まで回さないと鉤にならない。爪先を峰の 5cm 下、
       切っ先の上の角より上に置くと、爪先と切っ先の前面のあいだに口の開いた懐ができる。 */
    /* ※ ここまでで3回作り直している。学んだのは3つ。
       (a) 峰の延長線上へ真っ直ぐ尖らせると「槍先」。曲げないと鉤にならない。
       (b) 下へ曲げるだけでは足りない。**先端を持ち手の側へ戻す**ところまで回して初めて、
           「引っ掛けて手前に引く」形として読める。
       (c) 爪の内側と、鉈本体の切っ先の前面を**一続きの曲線で繋ぐ**こと。
           角で突き合わせると「板の角に別の部品を付けた」ように見える（agy）。 */
    /* (d) 巻き込みすぎると今度は**鳥のクチバシ**になる。「引っ掛けた枝が噛み込んで
           抜けなくなりそう」に見える（agy）。実物の爪は小ぶりで、懐は少し開いている。
           太さ・突き出し・巻き込みの3つを一度に落とす（1つだけ削ると輪郭が破綻する）。 */
    /* (e) 深く巻いた版は「過剰・クチバシ」、浅くした版は「足りない・ちょこんと付いている」と、
           **同じ評価者が逆のことを言った**。これは計測器の底なので、判定を委ねずに
           2版の中間を取る（HANDOFF「矛盾する指摘は自分で決める」）。
           両方の回で一致していた指摘は「爪先が丸い」の1点だけなので、そこだけ効かせる。
           爪先は line ではなく2本の曲線を突き合わせて、鋭角の頂点として作る。 */
    bs.quadraticCurveTo(0.052, -0.670, 0.056, -0.767);          // 爪の背（峰から滑らかに立ち上がる）
    bs.quadraticCurveTo(0.044, -0.816, -0.038, -0.786);         // 爪先（刃の側へ回り込んで尖る）
    bs.quadraticCurveTo(-0.020, -0.748, 0.017, -0.704);         // 爪の懐（内側・引っ掛ける面）
    bs.quadraticCurveTo(-0.002, -0.690, -0.120, -0.684);        // 懐から切っ先の前面へ一続きに繋ぐ
    bs.lineTo(-0.186, -0.680);
    bs.quadraticCurveTo(-0.232, -0.674, -0.242, -0.596);        // 切っ先の下の角
    ...
    rbox(0.012, 0.300, 0.018, 0.003, edgeM,    { x: -0.226, y: -0.396, z: 0, pre: W });  // 研いだ刃先
