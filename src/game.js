...
function makeMonster() {
  ...
  const BONES = [
    ...
    [0, 1.259, 0.077, 0, 1.378, 0.224, 0.180],   // 背骨 32°→64°（中背）
    [0, 1.378, 0.224, 0, 1.410, 0.407, 0.196],   // 背骨 64°→94°（背の峰＝肩の山）
    [0, 1.410, 0.407, 0, 1.384, 0.535, 0.100],   // 背骨 94°→116°（肩甲骨のヨーク／首の手前で絞る）
  ...
  function apronSheet(wChest, wWaist, wHem, h, bow, sway, mat, o, tWaist = 0.56, folds = 4, amp = 0.014) {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      const curl = -Math.max(0, Math.abs(u) - 0.34) * 1.9 * bow;
  ...
  const HIP  = { y: 1.088, z: 0.020 };            // 股関節＝弧の始点
  ...
  const HEAD = { y: 1.286, z: 0.800 };   // 頭の中心（肩より前・少し下）
  ...
  for (const s of [-1, 1]) {
    ...
    const kn = { x: s * 0.286, y: 0.600,  z: HIP.z + kz };
    const an = { x: s * 0.322, y: 0.112,  z: HIP.z + az };
  ...
  sph(0.145, bodyM, { y: 0.975, z: -0.045, sx: 1.22, sy: 0.92, sz: 1.00, uv: 1.8, weld: true }, 26, 16);  // 尻（骨盤の後ろ下）
  {
    ...
    const RAD = [0.170, 0.182, 0.196, 0.100];   // 最終区間＝ヨーク端は首の手前で絞る
    for (let i = 0; i < 4; i++) {
  ...
  tor(0.124, 0.011, cordM, { y: HEAD.y + 0.046, z: HEAD.z, rx: Math.PI / 2, sy: 1.42, uv: 2.0, pre: headM });
  ...
  const arm = (s, grip) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (grip) {
        cap(0.0125 * t, 0.030 * t, bodyDk, { x: fx, y: P.y - 0.030, z: P.z + 0.040, rx: 1.20, uv: 0.8 }, 8);
        cap(0.0115 * t, 0.026 * t, bodyDk, { x: fx, y: P.y - 0.062, z: P.z + 0.034, rx: 0.25, uv: 0.8 }, 8);
      } else {
  ...
  apronSheet(0.244, 0.300, 0.452, 0.960, 0.118, -0.135, canvasM,
             { y: 0.760, z: 0.512, uv: 1.0 }, 0.60, 5, 0.020);
  ...
  for (const sd of [-1, 1]) {
    ...
    const st = bone3({ x: sd * 0.150, y: 1.292, z: 0.578 },    // 胸当ての上角（布の面の上）
                     { x: sd * 0.105, y: 1.432, z: 0.502 });   // ヨークの上（首の付け根）
    cap(0.014, st.len, strapM, { ...st, uv: 1.0 }, 8);
    rbox(0.030, 0.030, 0.010, 0.004, strapM, { x: sd * 0.150, y: 1.288, z: 0.586 });   // 胸当てに紐を留める金具
  }
  ...
  arc(0.578, 0.578, 0.016, twineM, { y: 1.012, z: 0.062, uv: 1.0 }, -0.345, 0.690, 24);   // 腰帯（前掛けの曲面に沿わせる）
  ...
  for (const sd of [-1, 1]) {
    ...
    const bx = sd * 0.104, by = 0.882, bz = 0.512;
  ...
  const bRoll = 0.61;                                             // 長さ軸まわりの転がし角（rad）
  ...
  const bO = new THREE.Vector3(GRIP.x + 0.020, GRIP.y + 0.235, GRIP.z + 0.300);
     ...
     腕の位置を動かしても柄が宙に浮かない。 */
  {
    ...
    cyl(0.027, 0.023, hb.len + 0.10, handleM, { ...hb, uv: 1.0 }, 14);   // 柄（拳を貫通させる）
    const pom = { x: GRIP.x + (GRIP.x - A.x) / hb.len * 0.062,
                  y: GRIP.y + (GRIP.y - A.y) / hb.len * 0.062,
                  z: GRIP.z + (GRIP.z - A.z) / hb.len * 0.062 };
  ...
  rbox(0.026, 0.470, 0.030, 0.006, cleaverM, { x: 0.012, y: -0.330, z: 0, pre: mac });
