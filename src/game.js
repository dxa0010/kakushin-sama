...
function makeMonster() {
   ...
   */
  function apronSheet(wChest, wWaist, wHem, h, bow, mat, o, tWaist = 0.56, folds = 4, amp = 0.014) {
    const geo = new THREE.PlaneGeometry(1, h, 30, 26);
    const pos = geo.attributes.position;
    for (let i = 0; i < pos.count; i++) {
      ...
      // 縦ジワ。腰紐の位置では布が吸い込まれるので一度浅くする
      const cinch = Math.exp(-(((v - tWaist) / 0.10) ** 2));
      const wrink = amp * Math.sin(folds * Math.PI * u * 2) * (1 - 0.75 * cinch);
      pos.setXYZ(i, u * w, y, swell + wrink);
    }
    geo.computeVertexNormals();
    put(geo, mat, o);
  }
  ...
  const HEAD = { y: 1.268, z: 0.874 };   // 頭の中心（肩よりずっと前・低い位置）
  ...
  for (const s of [-1, 1]) {
    ...
    const kn = { x: s * 0.230, y: 0.600,  z: HIP.z + kz };
    const an = { x: s * 0.240, y: 0.112,  z: HIP.z + az };
    ...
    sph(0.088, jointM, hp, 22, 14);                                                       // 股関節の球
    cyl(0.062, 0.084, th.len, bodyM, { ...th, uv: 1.6 }, 20);                             // 腿（根元 0.084 → 膝 0.062）
    sph(0.064, jointM, kn, 20, 14);                                                       // 膝の球
    cyl(0.040, 0.058, sk.len, bodyM, { ...sk, uv: 1.6 }, 20);                             // すね（膝 0.058 → 足首 0.040）
    sph(0.044, jointM, an, 16, 12);                                                       // 足首の球
  ...
  const arm = (s, grip) => {
    ...
    const SH = { x: s * 0.332, y: 1.318, z: 0.352 };   // 肩（三角筋の中心）
    const EL = { x: s * 0.356, y: 0.868, z: 0.268 };   // 肘（後ろへ張り出す）
    const WR = { x: s * 0.340, y: 0.438, z: 0.352 };   // 手首（前へ戻る）
    const ua = bone3(SH, EL), fo = bone3(EL, WR);
    sph(0.080, jointM, SH, 22, 14);                                          // 肩の球（三角筋）
    cyl(0.056, 0.076, ua.len, bodyM, { ...ua, uv: 1.6 }, 20);                // 上腕（肩 0.076 → 肘 0.056）
    sph(0.060, jointM, EL, 20, 14);                                          // 肘の球
    cyl(0.038, 0.054, fo.len, bodyM, { ...fo, uv: 1.6 }, 20);                // 前腕（肘 0.054 → 手首 0.038）
    sph(0.042, jointM, WR, 16, 12);                                          // 手首の球
  ...
  apronSheet(0.244, 0.300, 0.452, 0.960, 0.112, canvasM,
             { y: 0.760, z: 0.462, rx: 0.16, uv: 1.0 }, 0.60, 4, 0.014);
  ...
  const bDir = new THREE.Vector3(0.62, -0.60, 0.34).normalize();   // 付け根 → 切っ先
  ...
  const bO = new THREE.Vector3(GRIP.x + 0.048, GRIP.y + 0.075, GRIP.z + 0.022);
