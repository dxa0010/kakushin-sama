  ...
  /* 側面へ垂れる掛け布団の端（左右）
     ...
     直し方は3点。
       ① 垂れの起点を作る: マットレス側面の外側へ「縁の巻き込み」を置く。これは
          ...
          重なっても貫通しない。
       ② 垂れ布はフレームの外側だけを占める薄い板にする（右 x≦-6.048、左 x≧-7.05）。
       ③ 脚の z（-3.85 / -5.55）を避けた z 帯（-4.5〜-4.0）に収める。
     いずれも「重なっていないこと」を数値で確かめられるように寸法をコメントに残す。 */
  rbox(0.10, 0.10, 0.42, M.blanket, -6.07, 0.60, -4.20, 0, 0.045, 4);    // 右: 縁の巻き込み（x -6.12〜-6.02）
  const flap = rbox(0.045, 0.40, 0.40, M.blanket, -6.010, 0.40, -4.20, 0, 0.022, 4);
  flap.rotation.z = 0.08;                                                // 裾を少し外へ振る（x -6.048〜-5.972）
  rbox(0.09, 0.09, 0.34, M.blanket, -7.025, 0.59, -4.34, 0, 0.04, 4);    // 左: 縁の巻き込み（x -7.07〜-6.98）
  const flap2 = rbox(0.04, 0.32, 0.34, M.blanket, -7.085, 0.42, -4.34, 0, 0.02, 4);
  flap2.rotation.z = -0.09;                                              // （x -7.119〜-7.051）
  /* 枕2つ（ヘッドボード際に横並び）
     ------------------------------------------------------------------
