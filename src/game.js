...
function makeMonster() {
  ...
  const HEAD = { y: 1.386, z: 0.664 };   // 頭の中心（ヨークの前・やや上）
  ...
  for (const s of [-1, 1]) {
    ...
    /* 【靴が丸い塊だった】箱1個と球1個では、甲・爪先・踵の区別が出ない。
       参照の靴は「低い甲」「前へ伸びた爪先」「一段下がった踵」の3つの塊で出来ている。
       部品を割って段差を作ると、同じ大きさでも靴として読める。 */
    /* 【面が重なって明滅していた】靴底(0.002〜0.030)と甲(0.029〜0.103)の境界が
       1mm しか重ならず、ほぼ同一平面になって z ファイティングを起こしていた
       （足元に赤緑の線が出る）。部品どうしは**十分に食い込ませる**こと。
       接するだけ・僅かに重なるだけの配置は、必ずちらつく。 */
    rbox(0.126, 0.026, 0.286, 0.012, bodyDk, { x: an.x + s * 0.006, y: 0.015, z: an.z + 0.030, ry: s * 0.17, uv: 1.0 });  // 靴底
    rbox(0.120, 0.092, 0.176, 0.028, bodyDk, { x: an.x + s * 0.006, y: 0.064, z: an.z - 0.006, ry: s * 0.17, uv: 1.0 });  // 甲（低く平ら）
    rbox(0.104, 0.046, 0.110, 0.022, bodyDk, { x: an.x + s * 0.020, y: 0.044, z: an.z + 0.132, ry: s * 0.17, uv: 1.0 });  // 爪先
    rbox(0.094, 0.052, 0.072, 0.016, bodyDk, { x: an.x - s * 0.002, y: 0.050, z: an.z - 0.088, ry: s * 0.17, uv: 1.0 });  // 踵（一段下がる）
  }
  ...
  tor(0.164, 0.012, cordM, { y: HEAD.y + 0.052, z: HEAD.z, rx: Math.PI / 2, sy: 1.00, uv: 2.0, pre: headM });
  ...
  /* 内側 0.34・下 0.62・前 0.71。内側を抑えると刃幅の方向 wX の y 成分が 0.48 に下がり、
     刃の下の角の垂れが 0.097 で済む。 */
  const WDIR  = new THREE.Vector3(0.34, -0.62, 0.71).normalize();
  const WROLL = 0.15;
  const arm = (s, grip) => {
    ...
    sph(0.052, bodyDk, { x: P.x, y: P.y, z: P.z, sx: 0.86, sy: 1.30, sz: 1.05, rx: 0.08, uv: 1.0 }, 18, 12);   // 手のひら
    ...
    if (grip) {
      ...
      /* 柄の軸は武器の -WDIR（＝握りから柄頭へ向かう向き）。武器と同じ値を使うので、
         刃の角度を変えても指は必ず柄を包む。 */
      const ux = -WDIR.x, uy = -WDIR.y, uz = -WDIR.z;
      ...
      for (let i = 0; i < 4; i++) {
        ...
        /* 【マジックハンドに見えた】半径 0.058 は柄（0.033）の 1.8 倍で、指だけが
           大きく外へ張り出して機械の爪のようになっていた（agy）。
           柄の表面に触れる 0.042 まで詰め、手のひらの塊で付け根を隠す。 */
        const R = 0.042;                            // 柄の半径ぶん外へ
  ...
  for (const sd of [-1, 1]) {
    ...
    /* 角丸を 0.006 → 0.018 に。直方体のままだと「箱」にしか見えない。
       紙の束は角が潰れて丸い。 */
    rbox(0.146, 0.184, 0.044, 0.018, sheafM,
    ...
    cap(0.0115, 0.196, twineM, { x: bx, y: by, z: bz + 0.026, rz: Math.PI / 2, ry: sd * 0.20, uv: 1.0 }, 8);   // 横に回した紐
    cap(0.0115, 0.238, twineM, { x: bx, y: by, z: bz + 0.026, ry: sd * 0.20, uv: 1.0 }, 8);                    // 縦に回した紐
  }
     ...
     組み立て、最後に1つの行列で手へ持たせる。刃渡りや角度を変えても、柄と指の関係は崩れない。 */
  {
    ...
    cyl(0.033, 0.030, 0.240, handleM, { y: 0.010, pre: W }, 16);      // 柄（拳の中心を貫通する）
    sph(0.038, handleM, { y: 0.128, pre: W }, 14, 10);                 // 柄頭
    for (const ty of [-0.028, 0.046]) {                                // 革巻き（握る場所を示す）
      sph(0.037, strapM, { y: ty, sy: 0.34, pre: W }, 14, 8);
    }
    rbox(0.072, 0.024, 0.030, 0.006, bladeM, { y: -0.096, pre: W });   // 口金

    /* --- 刃 --- 原点から下へ伸ばす。刃渡りは握りから 0.62m。
       参照の角度（水平から 36 度）で下ろすと落差 0.365m で、拳(y≒0.38)から床に触れる。 */
    const bs = new THREE.Shape();
    bs.moveTo(0.022, -0.108);                                   // 峰（柄側）
    bs.lineTo(0.022, -0.404);                                   // 峰（先端側）
    bs.quadraticCurveTo(0.022, -0.466, -0.046, -0.470);         // 先端の上の角
    bs.lineTo(-0.142, -0.470);
    bs.quadraticCurveTo(-0.201, -0.470, -0.203, -0.418);        // 先端の下の角（大きく丸める）
    bs.lineTo(-0.198, -0.108);                                  // 刃（柄側）
    bs.closePath();
       ...
       押し出したあと頂点の z を刃幅方向の位置に応じて縮める。 */
    {
      ...
      const EDGE = -0.203, SPINE = 0.022, MID = 0.010;
      ...
      put(g2, cleaverM, { z: -0.010, pre: W });
    }
    rbox(0.024, 0.316, 0.032, 0.006, cleaverM, { x: 0.010, y: -0.262, z: 0, pre: W });   // 峰の厚み
    rbox(0.012, 0.310, 0.018, 0.003, edgeM,    { x: -0.194, y: -0.264, z: 0, pre: W });  // 研いだ刃先
    tor(0.019, 0.005, cleaverM, { x: -0.056, y: -0.400, z: 0.0, pre: W }, 12);           // 先端の穴のふち
  }
  ...
  const FR = 0.172, FPW = 0.278, FPH = 0.300;    // 巻き付ける半径・紙の幅・高さ
