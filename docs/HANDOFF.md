...
**v9で何が変わったか（要約）**: 全アイテム（家具9点＋怪人）の高精細化が完了し、目標品質レベル6〜7に到達。その後、部屋の明るさを初期比1/4まで落とし、デスクライトを消灯、PC画面を机の主光源化、アイテムマーカーを減光・縮小した。**照明を触る場合は必ず「7. 照明の設計と調整方法」を先に読むこと**（明るさの実体は初期定義ではなく `applyLights()` にあり、体感の明るさを最も強く支配しているのはプレイヤー視点の懐中電灯である）。
